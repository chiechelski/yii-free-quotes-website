...
class User extends CActiveRecord
{
     ...
     * @return \user|null 
     */
    public static function prepareUserForAuthorisation($username, $type = '')
    {
        if (strpos($username, "@")) 
        {
            $user = self::model()->notDeleted()->find('LOWEsR(e_mail) = ? AND ApplicationType = ?', array($username, $type));
        } 
        else 
        {
            $user = self::model()->createRandomUsername()->find('LOWER(username)=?', array($username));
        }
